  ...
  <div class="container-fluid py-4">
    <div class="d-flex flex-wrap justify-content-between align-items-end gap-3 mb-4">
      <div>
        ...
        <p class="text-muted mb-0">One catalogue now controls calculator suggestions, stock availability, quotation demand and restocking.</p>
      </div>
    ...
    <div class="alert alert-info planning-note">
      ...
      <strong>Planning logic:</strong> approved, scheduled and in-progress projects are committed demand. Pending and sent quotations are forecast demand. Restock priority considers both demand groups, safety stock and supplier lead time.
    </div>
    <div v-if="hasLegacyFallback" class="alert alert-warning">
      Legacy inverter or battery records are being shown as out of stock. Import them once, then enter actual quantities, supplier and reorder settings.
    </div>
      ...
      <div class="card-body">
        <form @submit.prevent="saveItem">
          ...
          <div v-if="form.type === 'panel'" class="row g-3">
            ...
            <div class="col-md-8"><label class="form-label">Technology</label><input v-model.trim="form.specs.technology" class="form-control" placeholder="HJT, TOPCon, Mono PERC…" /></div>
          </div>
          ...
          <div v-if="isAccessoryType" class="row g-3">
            <div class="col-12">
              <div class="form-check form-switch">
                <input id="autoInclude" v-model="form.specs.autoInclude" class="form-check-input" type="checkbox" />
                <label for="autoInclude" class="form-check-label">Automatically include this item in every recommended system</label>
              </div>
            </div>
          ...
          <div class="row g-3 mt-2">
            <div class="col-md-6">
              <div class="profit-preview h-100">
            ...
            <div class="col-md-6 d-flex align-items-center">
              <div class="form-check form-switch">
                <input id="discontinued" v-model="form.discontinued" class="form-check-input" type="checkbox" />
                <label for="discontinued" class="form-check-label">Discontinued — exclude from recommendations and restock orders</label>
              </div>
            </div>
          </div>
    ...
    <div class="card mb-4">
      <div class="card-header border-bottom-0 pb-0">
        <ul class="nav nav-tabs card-header-tabs">
          <li class="nav-item"><button type="button" :class="['nav-link', { active: activeTab === 'stock' }]" @click="activeTab = 'stock'">Inventory catalogue</button></li>
          <li class="nav-item"><button type="button" :class="['nav-link', { active: activeTab === 'restock' }]" @click="activeTab = 'restock'">Restock planner <span v-if="restockItems.length" class="badge bg-danger ms-1">{{ restockItems.length }}</span></button></li>
        </ul>
      </div>
      <div class="card-body border-bottom">
        <div class="row g-3 align-items-end">
          <div class="col-lg-4"><label for="inventorySearch" class="form-label">Search</label><input id="inventorySearch" v-model.trim="searchQuery" type="search" class="form-control" placeholder="Name, SKU, supplier or type" /></div>
          <div class="col-sm-6 col-lg-2"><label class="form-label">Type</label><select v-model="typeFilter" class="form-select"><option value="all">All types</option><option v-for="type in itemTypes" :key="type.value" :value="type.value">{{ type.label }}</option></select></div>
          <div class="col-sm-6 col-lg-2"><label class="form-label">Priority</label><select v-model="priorityFilter" class="form-select"><option value="all">All priorities</option><option value="critical">Critical</option><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option></select></div>
          ...
          <div class="col-sm-6 col-lg-2"><button type="button" class="btn btn-outline-secondary w-100" :disabled="loading" @click="loadPlan"><i class="fas fa-rotate me-2" :class="{ 'fa-spin': loading }"></i>Refresh</button></div>
        </div>
      </div>
        ...
        <table class="table table-hover align-middle mb-0 smart-stock-table">
          ...
          <tbody>
            <tr v-for="item in filteredItems" :key="item.id">
              <td><strong>{{ item.name }}</strong><br /><small class="text-muted">{{ item.sku || 'No SKU' }} · {{ typeLabel(item.type) }}<span v-if="item.supplier"> · {{ item.supplier }}</span></small></td>
              ...
              <td><strong>{{ numberValue(item.quantity) }}</strong> {{ item.unit }}</td>
              <td>{{ numberValue(item.committedDemand) }}</td>
              <td>{{ numberValue(item.quotationDemand) }}</td>
              <td><strong :class="numberValue(item.availableAfterCommitted) <= numberValue(item.reorderPoint) ? 'text-warning' : 'text-success'">{{ numberValue(item.availableAfterCommitted) }}</strong></td>
              <td><strong :class="numberValue(item.projectedShortfall) > 0 ? 'text-danger' : 'text-success'">{{ numberValue(item.projectedShortfall) }}</strong></td>
              <td><span class="priority-chip" :class="`is-${item.priority}`">{{ priorityLabel(item.priority) }}</span><br /><small class="text-muted">Score {{ numberValue(item.priorityScore) }}</small></td>
              <td v-if="canWriteInventory" class="text-nowrap">
                <button v-if="!item.legacySource || item.createdAt" class="btn btn-sm btn-outline-secondary me-1" :disabled="busy" aria-label="Edit inventory item" @click="editItem(item)"><i class="fas fa-pen"></i></button>
                <button v-if="!item.legacySource || item.createdAt" class="btn btn-sm btn-outline-danger" :disabled="busy" aria-label="Delete inventory item" @click="removeItem(item)"><i class="fas fa-trash"></i></button>
                <span v-else class="small text-warning">Import first</span>
              </td>
            </tr>
      ...
      <div v-else class="table-responsive">
        <table class="table table-hover align-middle mb-0">
          <thead><tr><th>Restock rank</th><th>Item</th><th>Why it is urgent</th><th>Shortfall</th><th>Recommended order</th><th>Lead time</th><th>Supplier</th></tr></thead>
          <tbody>
            <tr v-for="(item, index) in filteredRestockItems" :key="item.id">
              <td><span class="rank-badge">#{{ index + 1 }}</span></td>
              <td><strong>{{ item.name }}</strong><br /><small class="text-muted">{{ item.sku || 'No SKU' }} · {{ typeLabel(item.type) }}</small></td>
              <td>
                <span class="priority-chip me-2" :class="`is-${item.priority}`">{{ priorityLabel(item.priority) }}</span>
                <small>{{ restockReason(item) }}</small>
              </td>
              <td><div>Committed: <strong :class="item.committedShortfall > 0 ? 'text-danger' : ''">{{ item.committedShortfall }}</strong></div><div>Quotation: <strong :class="item.quotationShortfall > 0 ? 'text-warning' : ''">{{ item.quotationShortfall }}</strong></div></td>
              <td><strong class="fs-5">{{ item.recommendedOrder }}</strong> {{ item.unit }}</td>
              <td>{{ item.leadTimeDays }} day(s)</td>
              <td>{{ item.supplier || 'Supplier not assigned' }}</td>
            </tr>
            <tr v-if="!filteredRestockItems.length"><td colspan="7" class="text-center py-5"><div class="enterprise-empty-state py-2"><div class="enterprise-empty-state__icon"><i class="fas fa-circle-check"></i></div><h3 class="h6">No restock action for this filter</h3><p class="text-muted mb-0">Current stock covers committed and quotation demand plus configured stock policy.</p></div></td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    isAccessoryType() {
      return ['wiring', 'mounting', 'other'].includes(this.form.type);
    },
    profitPerUnit() {
    ...
    inventoryMetrics() {
      return [
        { label: 'Inventory items', value: this.numberValue(this.summary.totalItems), icon: 'fas fa-boxes-stacked', className: 'is-blue' },
        { label: 'Committed demand', value: this.numberValue(this.summary.committedDemand), icon: 'fas fa-lock', className: 'is-purple' },
        { label: 'Projected shortfall', value: this.numberValue(this.summary.projectedShortfall), icon: 'fas fa-triangle-exclamation', className: 'is-amber' },
        { label: 'Critical restocks', value: this.numberValue(this.summary.criticalItems), icon: 'fas fa-fire', className: 'is-red' }
      ];
    },
    filteredItems() {
      ...
      return this.items.filter(item => {
        ...
        const matchesPriority = this.priorityFilter === 'all' || item.priority === this.priorityFilter;
        const matchesStatus = this.statusFilter === 'all' || item.stockStatus === this.statusFilter;
        const matchesQuery = !query || [item.name, item.sku, item.supplier, item.description, item.type]
          .some(value => String(value || '').toLowerCase().includes(query));
        return matchesType && matchesPriority && matchesStatus && matchesQuery;
      });
    },
    restockItems() {
      return this.items.filter(item => !item.discontinued && this.numberValue(item.recommendedOrder) > 0);
    },
    filteredRestockItems() {
      const eligibleIds = new Set(this.filteredItems.map(item => item.id));
      return this.restockItems.filter(item => eligibleIds.has(item.id));
    }
  },
  created() {
    this.loadPlan();
  ...
  methods: {
    emptyForm() {
      return {
        type: '', sku: '', name: '', description: '', costPrice: 0, sellingPrice: 0,
        quantity: 0, reorderPoint: 5, targetStock: 10, leadTimeDays: 7,
        supplier: '', unit: 'piece', activeForCalculator: false, discontinued: false,
        specs: {
          wattage: null, technology: '', peakLoad: null, maxPanels: null,
          batterySupported: 0, capacity: null, energy: null, voltage: 12,
          fixedQuantityPerSystem: 0, perPanelQuantity: 0, autoInclude: false
        }
      };
    },
    numberValue(value) {
    ...
    restockReason(item) {
      if (item.committedShortfall > 0) return `${item.committedProjectCount} committed project(s) cannot be fully supplied.`;
      if (item.quotationShortfall > 0) return `${item.quotationProjectCount} quotation(s) create projected shortage.`;
      if (item.availableAfterCommitted <= item.reorderPoint) return 'Available stock is at or below the reorder point.';
      return 'Projected balance is below the configured target stock.';
    },
    toggleForm() {
    ...
    async loadPlan() {
      ...
      this.error = '';
      try {
        const result = await authenticatedJsonRequest('/.netlify/functions/getInventoryPlan', { method: 'GET' });
        this.items = Array.isArray(result.items) ? result.items : [];
        this.summary = result.summary || {};
        this.hasLegacyFallback = Boolean(result.hasLegacyFallback);
      } catch (error) {
        this.error = error.message || 'Unable to load inventory planning data.';
      } finally {
        this.loading = false;
      }
    },
    async migrateLegacy() {
      if (!this.canWriteInventory || !window.confirm('Import the old inverter and battery catalogue into inventory with quantity 0?')) return;
      this.busy = true;
      this.error = '';
      try {
        const result = await authenticatedJsonRequest('/.netlify/functions/migrateLegacyEquipment', { method: 'POST' });
        this.notify(result.message || 'Legacy equipment imported.');
        await this.loadPlan();
      } catch (error) {
    ...
    async saveItem() {
      ...
      this.error = '';
      try {
        const payload = { ...this.form, specs: { ...this.form.specs } };
        const result = this.editingItemId
          ? await updateInventoryItem(this.editingItemId, payload)
          : await createInventoryItem(payload);
        if (!result.success) throw new Error(result.error || 'Unable to save inventory item');
        this.notify(this.editingItemId ? 'Inventory item updated.' : 'Inventory item added.');
        this.resetForm();
        await this.loadPlan();
      } catch (error) {
        this.error = error.message;
    ...
    editItem(item) {
      if (!this.canWriteInventory || (item.legacySource && !item.createdAt)) return;
      ...
      this.form = {
        ...
        reorderPoint: this.numberValue(item.reorderPoint),
        targetStock: this.numberValue(item.targetStock),
        leadTimeDays: this.numberValue(item.leadTimeDays) || 7,
        supplier: item.supplier || '',
        unit: item.unit || 'piece',
        activeForCalculator: Boolean(item.activeForCalculator),
        discontinued: Boolean(item.discontinued),
        specs: { ...this.emptyForm().specs, ...(item.specs || {}) }
      };
      this.showForm = true;
      window.scrollTo({ top: 0, behavior: 'smooth' });
    },
    async removeItem(item) {
      if (!this.canWriteInventory || !window.confirm(`Delete ${item.name}? Existing project snapshots remain unchanged.`)) return;
      this.busy = true;
      this.error = '';
      try {
        const result = await deleteInventoryItem(item.id);
        if (!result.success) throw new Error(result.error || 'Unable to delete item');
        this.notify('Inventory item deleted.');
        await this.loadPlan();
      } catch (error) {
        this.error = error.message;
      } finally {
...
<style scoped>
.planning-note { border-left: 4px solid var(--ant-blue-600); }
.inventory-metric__icon { flex: 0 0 auto; }
.profit-preview { display: flex; justify-content: space-around; gap: 1rem; padding: 1rem; border-radius: 12px; background: var(--ant-slate-50); border: 1px solid var(--ant-slate-200); }
.profit-preview div { display: flex; flex-direction: column; }
.priority-chip { display: inline-flex; padding: 0.3rem 0.55rem; border-radius: 999px; font-size: 0.75rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.03em; }
...
.rank-badge { display: inline-grid; place-items: center; width: 2.2rem; height: 2.2rem; border-radius: 50%; background: var(--ant-slate-900); color: #fff; font-weight: 800; }
.smart-stock-table th { white-space: nowrap; }
@media (max-width: 767.98px) { .profit-preview { flex-direction: column; } }
</style>
